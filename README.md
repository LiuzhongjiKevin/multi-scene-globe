# Multi-Scene Globe · 京都 · 二年坂与三年坂

以京都东山二年坂、三年坂及周边街道的地图记录为依据，重建石阶段、町屋、店铺与八坂塔，保留原版微缩街区形态。

这是 **`scene/kyoto`** 独立分支，运行依赖、模型和对应建模来源均已包含，无需下载其他分支。
...
京都原版街区全景 · Blender 离线结构渲染。

![三年坂石阶近景 · Blender 离线结构渲染](design-review/sannenzaka/sannen.png)

三年坂石阶近景 · Blender 离线结构渲染。

![二年坂街巷 · Blender 离线结构渲染](design-review/sannenzaka/ninen.png)

二年坂街巷 · Blender 离线结构渲染。
...
```sh
git clone --branch scene/kyoto --single-branch https://github.com/LiuzhongjiKevin/multi-scene-globe.git
...
| `dist/sannen-main.js` | 场景初始化、动画循环与交互入口 |
...
`design-review/sannenzaka/sannenzaka-assets.blend` 为京都建筑模型；同目录 `build_blender.py` 生成并导出建筑 GLB，`source.osm.xml.gz` 与 `extract_map.py` 保存地图来源和提取步骤。人物和自行车复用根目录 `yugure-form.blend`。
...
街道与建筑锚点来自 OpenStreetMap；立面、店内、地形高度和灯光为艺术重建，并非测绘级复刻。
