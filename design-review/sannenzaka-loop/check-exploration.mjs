...
export function checkExploration(d, dom, listeners, pointer, tick) {
  const checks=[],p=d.player,nav=p.nav,period=d.surface.length;
  const gap=(a,b)=>Math.hypot(Math.min(Math.abs(a.x-b.x),Math.abs(Math.abs(a.x-b.x)-period)),a.z-b.z);
  ...
  for(const road of d.town.roads) {
    ...
    for(let s=.06;s<=road.length+.06;s+=.06){
      ...
      assert(gap(pos,target)<.005,`Street ${road.id} blocked at ${s}`);
    }
    assert(Math.abs(pos.y-road.at(road.length).y)<.01);
    if(road.steps)for(let s=road.length;s>=0;s-=.06){const target=road.at(s);nav.move(pos,target.clone().sub(pos));assert(gap(pos,target)<.005,'Downstairs route blocked')}
  }
  ...
  checks.push(`All ${d.town.homes.length} open shop doors allow entry and exit; closed doors, walls, counters and scene edges block traversal`);
