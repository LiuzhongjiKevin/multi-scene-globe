...
export function buildDistrict(scene,town,library,props=null){
 ...
 for(let i=0;i<45;i++){const x=1+(maxx-2)*((i*.618033)%1),z=minz-14+(maxz-minz+28)*((i*.414213)%1),n=town.nearest(new T.Vector3(x,0,z));if(n.distance<9||n.distance>19||town.buildings.some(b=>Math.hypot(x-b.p.x,z-b.p.z)<Math.max(b.width,b.depth)*.65+2)||Math.hypot(x-town.data.pagoda[0],z-town.data.pagoda[2])<11)continue;tree(new T.Vector3(x,n.p.y,z),i%5===0,.8+(i%3)*.12)}
